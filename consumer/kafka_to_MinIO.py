...
"""
boto3 to connect to any S3 compatible storage like MinIO
KafkaConsumer to read messages from Kafka topics
pandas to handle data and write Parquet files
datetime to timestamp files
json to parse Kafka messages
...
# in kafka the data get setlized into binary bits so we need to decode it to utf-8 and load it as json (value_deserializer=lambda x: json.loads(x.decode('utf-8')))
# auto_offset_reset='earliest', : makes sure it reads from the beginning of the topic (of the stream) if no offset is committed yet
